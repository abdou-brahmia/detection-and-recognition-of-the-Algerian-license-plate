...
def sub_image(image_back,frame):
    ...
    new_image_temp= cv2.cvtColor(image_back_temp.copy(), cv2.COLOR_BGR2GRAY )
    somme=0

    for j in image_back_temp.shape[0]:
        for i in image_back_temp.shape[1]:
            new_image_temp[i,j]=abs(image_back[i,j]-frame_temp[i,j])
