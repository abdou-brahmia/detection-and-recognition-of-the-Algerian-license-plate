...
def take_back_ground_debut(cap,resolution,nombreFrameMaj):
    # nombreFrameMaj nombre des image pour calculler le background
    i=0
    tab_image=[]
...
def sub_image(image_back,frame,SEILLE_PIXEL): # return masck , nombre de pixel blanc
...
def verrifier(image_info,SEILLE_IMAGE_1,SEILLE_IMAGE_2): # True or false + zone si il ya
    stateValue=False
    image=image_info[0]
    ...
    if (image_info[1]>SEILLE_IMAGE_1):
        ...
        npaContours, npaHierarchy = cv2.findContours(image.copy(),             # entrer l'image, utiliser la copie, puisque cette function change cette image au cours de la recherche des contours
